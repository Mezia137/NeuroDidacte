...
class NeuralNetwork(object):
    """
    Represente le ResNet pour la politique et la valeur.
    Attributs:
        ...
        pi: Un tenseur TF pour les probabilites de recherche.
        v: Un tenseur TF pour les valeurs de recherche.
        states: Un tenseur TF avec les dimensions de la grille.
        training: Un scalaire booleen TF.
        train_pis: Un tenseur TF pour les probabilites cibles de recherche.
        train_vs: Un tenseur TF pour les valeurs cibles de recherche.
        loss_pi: Un tenseur TF pour la sortie de l'entropie croisee softmax sur pi.
        ...
        train_op: Un tenseur TF pour la sortie de l'optimiseur d'entrainement.
        saver: Un sauvegardateur TF pour ecrire les points de controle de l'entrainement.
...
class NeuralNetworkWrapper(object):
    ...
    def predict(self, state):
        ...
        state = state.tolist()
        state = state[0][0]+state[0][1]+state[0][2]
        return pi[0], v[0][0]
    ...
    def save_model(self, filename="current_model"):
        ...
        print("Enregistrement du modele:", filename, "a", CFG.model_directory)
    ...
    def load_model(self, filename="current_model"):
        ...
        print("Chargement du modele:", filename, "depuis", CFG.model_directory)
...
class Train(object):
    ...
    def play_game(self, game, training_data):
        ...
        while not game_over:
            ...
            if best_child == None:
                game_over, value = True, -1 * game.current_player 
                break
            ...
            game_over, value = game.check_winner(game.current_player)
...
class CFG(object):
    ...
    num_games = 100             # Nombre de parties jouees en auto-apprentissage pendant chaque iteration. Cela determine la quantite de donnees generees pour l'entrainement.     
    ...
    #parametre pour MTCS
    c_puct = 8                  # Niveau d'exploration utilise dans MCTS. Il controle le degre d'exploration versus l'exploitation dans l'algorithme de recherche MCTS.
    temp_init = 4               # Parametre de temperature initial pour controler l'exploration. Il est utilise dans les strategies d'exploration-exploitation dans l'apprentissage par renforcement.
    temp_final = 1            # Parametre de temperature final pour controler l'exploration. Il represente la temperature apres laquelle l'exploration est reduite.   
    temp_thresh = 3             # Seuil ou l'initialisation de la temperature passe a la finale. Il definit le point a partir duquel la temperature initiale est reduite jusqu'a la temperature finale.
    dirichlet_alpha = 3       # Valeur alpha pour le bruit de Dirichlet. Le bruit de Dirichlet est utilise pour augmenter la diversite des coups explores lors de la recherche MCTS.
    epsilon = 1.5               # Valeur d'epsilon pour calculer le bruit de Dirichlet. Il controle l'importance du bruit de Dirichlet dans la recherche MCTS.
    num_mcts_sims = 60          # Nombre de simulations MCTS (Monte Carlo Tree Search) par partie. MCTS est une technique utilisee dans les jeux pour decider du meilleur coup a jouer en simulant des sequences de jeu.

    l2_val = 0.1           # Niveau de regularisation des poids L2 utilise pendant l'entrainement. Il permet de limiter la complexite du modele et d'eviter le surapprentissage.
    ...
    learning_rate = 0.1         # Taux d'apprentissage pour l'optimiseur de momentum. Il controle la taille des pas effectues lors de la mise a jour des poids du reseau neuronal pendant l'entrainement.
    ...
    resnet_blocks = 3           # Nombre de blocs residuels dans le ResNet. Le ResNet est une architecture de reseau de neurones profonds utilisee dans de nombreuses taches de vision par ordinateur.
    ...
    model_directory = "./models/"       # Nom du repertoire pour stocker les modeles. Il specifie le repertoire ou les modeles entraines seront sauvegardes
    ...
    record_loss = 1             # Binaire pour enregistrer la perte de politique et de valeur dans un fichier. Il specifie si les pertes doivent etre enregistrees pendant l'entrainement.
...
class MonteCarloTreeSearch(object):
    ...
    def search(self, game, node, temperature):
        ...
        for _ in range(CFG.num_mcts_sims):
            ...
            _, wsa = game.check_winner(game.current_player)
